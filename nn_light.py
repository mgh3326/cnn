...
def inference(images, keep_prob, batch_size, image_crop_size, input_channels, num_of_classes, variable_with_weight_decay, variable_on_cpu, activation_summary, log_input, log_feature):
    ...
    with tf.variable_scope('local3') as scope:
        size = pool2.get_shape()[1] * pool2.get_shape()[2] * pool2.get_shape()[3]
        reshape = tf.reshape(pool2, tf.stack([-1, size]))
        weights = variable_with_weight_decay('weights', shape=[size, 64], stddev=0.05)
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = variable_with_weight_decay('wegiths', [32, num_of_classes], stddev=0.04)
